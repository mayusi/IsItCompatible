...
import io.github.mayusi.isitcompatible.ui.appupdate.AppInstallState
import io.github.mayusi.isitcompatible.ui.appupdate.PendingUpdate

/**
 * Patch-notes dialog shown when an app self-update is available.
 *
 * Composable params are bundled into [PendingUpdate] and [AppInstallState]
 * to stay well under the ~12-param limit that can trigger VerifyError on
 * some Dalvik/ART versions.
 *
    ...
    return buildAnnotatedString {
        ...
        for ((idx, line) in lines.withIndex()) {
            ...
            when {
                ...
                trimmed.isEmpty() -> {
                    // blank line → paragraph space (only if not the last line)
