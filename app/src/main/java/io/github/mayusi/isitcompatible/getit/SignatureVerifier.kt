 ...
 *
 * Two modes:
 *  1. Self-update (io.github.mayusi.isitcompatible): the downloaded APK must
 *     be signed with the same cert as the currently-running app. Android enforces
 *     this at install time anyway, but checking first gives a clear safety error
 *     instead of the cryptic "App not installed" system dialog.
 *  2. Other packages (emulators, fork): if the package is already installed,
 *     the downloaded APK's signer must match the installed version's signer — this
 *     prevents a malicious "update" swapping the signing key. If the package is
 *     NOT yet installed, there is no baseline to pin against; proceed with a
 *     logged note.
 *
 * No hardcoded cert hashes are used for third-party packages: we can't know them
 * ahead of time. For self-update the check is inherently key-agnostic — it just
 * asks "does the APK on disk match the key this very process was signed with?"
 */
